  ...
  //  ser_addr.sin_addr.s_addr = htonl(INADDR_ANY); //IP地址，需要进行网络序转换，INADDR_ANY：本地地址
   ...
   //    std::cout<<"scan_data_buf:   "<<scan_data_buf.frame_id;
    ...
    //     std::cout << scan_msg.ranges.size() << std::endl;
        ...
        scan_msg.range_min = scan_data_buf.range_min;
        scan_msg.range_max = scan_data_buf.range_max;
        scan_msg.ranges.reserve(897);    //预分配空间    vector 操作
        scan_msg.ranges.assign(&scan_data_buf.ranges[0],&scan_data_buf.ranges[896]);  //直接复制
        scan_msg.intensities.reserve(897);
        scan_msg.intensities.assign(&scan_data_buf.intensities[0],&scan_data_buf.intensities[896]);
